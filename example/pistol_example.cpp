...
void write(::garden::Pistol<std::string>* pistol, const std::string& name) {
    for (int i = 0; i < 100; ++i) {
        std::stringstream ss;
...
int main() {
    ...
    std::thread w1(write, &pistol, "w1");
    std::thread w2(write, &pistol, "w2");
    std::thread w3(write, &pistol, "w3");
    std::thread w4(write, &pistol, "w4");
    std::thread w5(write, &pistol, "w5");
